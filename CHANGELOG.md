...
## 2026-06-13 #feat scope:core scope:docs - 转"本地 fork"模式：内核直改 get_vram mps 分支 + Mac API 并发可调

- Why: 所有者拍板不顾上游、内核可改；pipeline/hybrid 在 transformers 5 下两处断点判死刑，主后端定为 vlm-auto-engine
- 详见: AGENTS.md "硬规则"/"transformers 5 兼容陷阱"/"get_vram"段 + mineru/utils/model_utils.py + mineru/cli/fast_api.py

## 2026-06-13 #perf scope:deploy - MPS 显存探测缺失修复：MINERU_VIRTUAL_VRAM_SIZE=16 固化进启动脚本

- Why: get_vram() 漏 mps 分支，32GB M4 被当 1GB → batch_ratio=1 GPU 空转；环境变量绕开免改内核
- 详见: AGENTS.md "get_vram() 不认 MPS"段 + scripts/local/{mineru-api,mineru-gradio}.sh
