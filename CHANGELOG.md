...
## 项目阶段
- [x] 本机环境跑通 — pipeline/vlm 后端 + demo 全样本
- [x] 算力服务部署 — 三段访问入口 + 反向 SSH 隧道
- [x] 持久化 + 公网入口 — LaunchDaemon + Traefik basic-auth
- [x] 双服务架构 — vlm 7861 / pipeline 7862（transformers 版本隔离）
- [x] 按文件类型分流 — apdd CLI + gradio UI 自动路由（端到端验证）
- [ ] 双服务持久化收尾 — pipeline 服务纳入 launchd + .venv-pipeline TCC FDA
